  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        setErrors({ general: 'Email/Password signup is not enabled in Firebase. Please enable it in the Firebase Console.' });
      } else {
        setErrors({ general: 'Failed to create account. Please try again or check your connection.' });
      }
    } finally {
      setSubmitting(false);
    }
        ...
        >
          <GlassCard className="p-6 md:p-7">
            ...
            <div className="space-y-4">
              ...
              {errors.general && (
                <div className="p-3 rounded-xl bg-accent/10 border border-accent/20 text-accent text-xs font-body">
                  {errors.general}
                </div>
              )}
