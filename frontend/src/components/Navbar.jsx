...
const Navbar = () => {
  ...
  const isDashboard = ['/dashboard', '/profile', '/settings', '/summarize', '/api-admin'].includes(
    location.pathname
  );
                ...
                {[
                  ...
                  return (
                    <NavLink
                      ...
                      to={link.href}
                      className={({ isActive }) => 
                        `text-sm font-body px-3 py-1.5 rounded-full transition-all duration-200 cursor-pointer ${
                          isActive
                            ? 'bg-gradient-to-r from-[#6C47FF]/20 to-[#00D4FF]/20 border border-[#6C47FF]/40 text-white shadow-[0_0_15px_rgba(108,71,255,0.3)]'
                ...
                <AnimatePresence>
                  {avatarOpen && (
                    <motion.div
                      ...
                      className="absolute right-0 mt-2 w-52 rounded-xl bg-surface/95 backdrop-blur-xl border border-border shadow-2xl py-2 z-50"
                    >
                      <div className="px-3 pb-2 border-b border-border/70 mb-1">
                        <p className="text-xs font-body text-muted">Signed in as</p>
                        <p className="text-sm font-body text-white truncate">{user?.email}</p>
                      </div>
                      <button
                        onClick={() => { setAvatarOpen(false); navigate('/summarize'); }}
                        className={`w-full px-3 py-2 text-left text-sm font-body transition-colors cursor-pointer ${
                          location.pathname === '/summarize' ? 'text-white bg-white/5' : 'text-muted hover:text-white hover:bg-white/5'
                        }`}
                      >
                        📄 Summarize
                      </button>
                      <button
                        ...
                        className={`w-full px-3 py-2 text-left text-sm font-body transition-colors cursor-pointer ${
                          location.pathname === '/profile' ? 'text-white bg-white/5' : 'text-muted hover:text-white hover:bg-white/5'
                        }`}
                      >
                        👤 Profile
                      </button>
                      <button
                        ...
                        className={`w-full px-3 py-2 text-left text-sm font-body transition-colors cursor-pointer ${
                          location.pathname === '/api-admin' ? 'text-white bg-white/5' : 'text-muted hover:text-white hover:bg-white/5'
                        }`}
                      >
                        🔑 API Admin
                      </button>
                      <button
                        ...
                        className={`w-full px-3 py-2 text-left text-sm font-body transition-colors cursor-pointer ${
                          location.pathname === '/settings' ? 'text-white bg-white/5' : 'text-muted hover:text-white hover:bg-white/5'
                        }`}
                      >
                        ⚙️ Settings
                      </button>
